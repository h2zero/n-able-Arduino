...
void tone(uint8_t pin, unsigned int frequency, unsigned long duration)
{
    ...
    if((frequency < 20) | (frequency > 25000)) {
        Serial.println("Tone - frequency outside range [20..25000] -- ignoring");
        return;
    }
    ...
    if (!_pwm_config.ensurePwmPeripheralOwnership()) {
        Serial.println("Tone - Unable to acquire PWM peripheral");
    } else if (!_pwm_config.stopPlayback(false)) {
        Serial.println("Tone - Unable to stop playback");
    } else if (!_pwm_config.initializeFromPulseCountAndTimePeriod(pulse_count, time_period)) {
        Serial.println("Tone - Failed calculating configuration");
    } else if (!_pwm_config.applyConfiguration(pin)) {
        Serial.println("Tone - Failed applying configuration");
    } else if (!_pwm_config.startPlayback()) {
        Serial.println("Tone - Failed attempting to start PWM peripheral");
    } else {
        //Serial.println("Tone - Started playback of tone at frequency %d duration %ld", frequency, duration);
    }
    return;
}
...
bool TonePwmConfig::ensurePwmPeripheralOwnership(void) {
    if (!_HwPWM->isOwner(TonePwmConfig::TONE_TOKEN) && !_HwPWM->takeOwnership(TonePwmConfig::TONE_TOKEN)) {
        Serial.println("Tone - unable to allocate PWM2 to Tone");
        return false;
    }
    return true;
}
...
bool TonePwmConfig::initializeFromPulseCountAndTimePeriod(uint64_t pulse_count_x, uint16_t time_period) {
    ...
    if (_bits_used(pulse_count_x) > 37) {
        Serial.println("Tone - pulse count is limited to 37 bit long value");
        return false;
    }
...
bool TonePwmConfig::startPlayback(void) {
    if (!this->ensurePwmPeripheralOwnership()) {
        Serial.println("Tone - PWM peripheral not available for playback");
        return false;
    }
...
bool TonePwmConfig::stopPlayback(bool releaseOwnership) {
    ...
    if (!_HwPWM->isOwner(TonePwmConfig::TONE_TOKEN)) {
      Serial.println("Tone - Attempt to set noTone when not the owner of the PWM peripheral.  Ignoring call....");
      return false;
    }
